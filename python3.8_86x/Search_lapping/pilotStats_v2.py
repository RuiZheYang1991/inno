...
class Stats:
    ...
    def __init__(self):
        ...
        self.ui.save.clicked.connect(self.save_date)
        ...
        self.res = pymssql.connect(server=host, user=user, password=pwd, database=db)
        ...
        if not (num.isdigit() and (len(num)==8)):
            ...
            try:
                ...
                for name,number,io,seat,timee in zip(n,nu,i,s,t):
                        ...
                        if key%2==0:
                            for it in all:
                                if ((self.NG_time_zoon[0]-buffer*10000)<int(it[2])<(self.NG_time_zoon[1]+buffer*10000)) and (it[3] in self.NG_seat):
                                    ...
                                    data_exc = [it[0],it[1],it[2],it[3]]
                                    save.append(data_exc)
                                    # name_l.append()
                                    # num_l.append()
                                    # time_l.append()
                                    # seat_l.append()
                    ...
                    self.ui.progressBar.setVisible(False)
                    self.ui.progressBar.reset()
                df = pd.DataFrame(save, columns=["Name", "Number", 'Time', 'Seat'])  # 指定欄位名稱
...
if __name__=='__main__':
    ...
    # pwd = config['SQL']['pwd']
    # db = config['SQL']['db']
    #host="PC89600071905\SQLEXPRESS"
    host='LAPTOP-IJV23O75\SQLEXPRESS'
    user="sa"
    pwd='P@ss1234'
    db='CLTestDB'
