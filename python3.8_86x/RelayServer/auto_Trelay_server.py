...
def handle():#client,addr
    ...
    while True:
        ...
        try:
            ...
            ptext=text.decode(codee)

            #ptext = "501,0000719991,I,P,36.6"
            #temp_dict["1"]="39.5"
            ...
            if len(ptext_list) == 5:
                ...
                try:
                    tttext=ptext_list[0]+","+ptext_list[1]+",I,P,36.5"
                    client.send(tttext.encode(codee))
                    stats.send_name(ptext_list[-1], loc_num)
                    stats.send_num(ptext_list[3], loc_num)
                    stats.send_temp('36.5', loc_num)
                ...
                try:
                    client.send('資料格式錯誤'.encode(codee))
            ...
            try:
                client.send("somethings wrong".encode(codee))
                client.close()
            except:
                pass
            #break
...
class Stats:
    ...
    def __init__(self):
        self.ui = QUiLoader().load('ui/static_temp.ui')
